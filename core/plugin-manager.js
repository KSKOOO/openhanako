...
export class PluginManager {
  ...
  async _loadRoutes(entry) {
    ...
    // Use `all("*", ...)` instead of `use()` because the packaged Linux
    // runtime can crash inside Hono's private route helper when `use()` is
    // invoked from bundled server code.
