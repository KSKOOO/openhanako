...
function getAutoUpdaterSupport() {
  if (!app.isPackaged) {
    return { supported: false, reason: "not_packaged" };
  }
  if (isRunningFromDmg()) {
    return { supported: false, reason: "running_from_dmg" };
  }
  if (process.platform === "linux" && !process.env.APPIMAGE) {
    return { supported: false, reason: "linux_manual_update" };
  }
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("auto-update-check", async () => {
    const support = getAutoUpdaterSupport();
    if (!support.supported) {
      if (support.reason && support.reason !== "not_packaged") {
        setState({ status: "error", error: support.reason });
      }
      return getState();
    }
...
function initAutoUpdater(mainWindow, {
  ...
  if (!support.supported) {
    if (support.reason && support.reason !== "not_packaged") {
      logUpdate(`auto-updater disabled: ${support.reason}`);
      setState({ status: "error", error: support.reason });
    }
    return;
  }
...
async function checkForUpdatesAuto() {
  if (!getAutoUpdaterSupport().supported) return;
