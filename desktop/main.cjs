...
const APP_PACKAGE_NAME = String(APP_PACKAGE_META.name || "hanako").toLowerCase().replace(/[^a-z0-9+.-]+/g, "-");
const APP_PRODUCT_NAME = String(APP_PACKAGE_META.build?.productName || APP_PACKAGE_META.productName || "Hanako");
const APP_DESKTOP_NAME = String(APP_PACKAGE_META.desktopName || `${APP_PACKAGE_NAME}.desktop`);
const APP_LINUX_WM_CLASS = String(
  APP_PACKAGE_META.build?.linux?.desktop?.entry?.["StartupWMClass"]
  || APP_PACKAGE_META.build?.linux?.desktop?.entry?.["X-GNOME-WMClass"]
  || APP_PACKAGE_NAME
);
let _cachedWindowIcon = undefined;

function resolveWindowIconPath() {
  const candidates = [];
  const platformIconFiles = process.platform === "linux"
    ? ["icon.png", "icon.ico"]
    : ["icon.ico", "icon.png"];
  if (app.isPackaged && process.resourcesPath) {
    for (const iconFile of platformIconFiles) {
      candidates.push(path.join(process.resourcesPath, "app.asar.unpacked", "desktop", "src", iconFile));
    }
  }
  for (const iconFile of platformIconFiles) {
    candidates.push(path.join(__dirname, "src", iconFile));
  }
...
function windowIconOpts() {
  ...
  if (_cachedWindowIcon !== undefined) {
    return _cachedWindowIcon ? { icon: _cachedWindowIcon } : {};
  }

  const iconPath = resolveWindowIconPath();
  if (!iconPath) {
    _cachedWindowIcon = null;
    return {};
  }

  const icon = nativeImage.createFromPath(iconPath);
  _cachedWindowIcon = icon && !icon.isEmpty() ? icon : null;
  return _cachedWindowIcon ? { icon: _cachedWindowIcon } : {};
}
...
if (process.platform === "linux") {
  process.env.CHROME_DESKTOP = process.env.CHROME_DESKTOP || APP_DESKTOP_NAME;
  try { app.setName(APP_PRODUCT_NAME); } catch {}
  try { app.commandLine.appendSwitch("class", APP_LINUX_WM_CLASS); } catch {}
  if (typeof app.setDesktopName === "function") {
    try { app.setDesktopName(APP_DESKTOP_NAME); } catch {}
  }
...
function resolvePackagedServerEntry(serverRoot) {
  const sourceEntry = path.join(serverRoot, "server", "index.js");
  if (fs.existsSync(sourceEntry)) return sourceEntry;
  return path.join(serverRoot, "bundle", "index.js");
}
...
async function _spawnServerOnce(serverInfoPath) {
  ...
  if (fs.existsSync(bundledServer) || fs.existsSync(bundledServer + ".exe")) {
    ...
    const entry = resolvePackagedServerEntry(bundledServerRoot);
    serverBin = bin;
...
function buildServerCrashDiagnostics() {
  ...
  const sourceEntryPath = isPackaged
    ? path.join(serverDir, "server", "index.js")
    : path.join(serverDir, "index.js");
  const bundlePath = isPackaged
    ? path.join(serverDir, "bundle", "index.js")
    : path.join(path.dirname(serverDir), "dist-server-bundle", "index.js");
  const selectedEntry = isPackaged ? resolvePackagedServerEntry(serverDir) : sourceEntryPath;
  ...
  const items = [
    ...
    `Packaged: ${!!isPackaged}`,
    `server/index.js exists: ${fs.existsSync(sourceEntryPath)}`,
    `bundle/index.js exists: ${fs.existsSync(bundlePath)}`,
    `selected server entry: ${selectedEntry}`,
