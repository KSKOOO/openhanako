...
const labels: Record<string, string> = {
  ...
  'settings.about.updateManualLinux': '当前 Linux 安装包不支持应用内更新，请下载新的 .deb 或 AppImage 包后手动安装',
};
...
describe('AutoUpdateStatus', () => {
  ...
  it('renders installing and dmg install guidance without a modal contract', () => {
    ...
    rerender(<AutoUpdateStatus state={updateState({ status: 'error', error: 'linux_manual_update' })} />);
    expect(screen.getByText('当前 Linux 安装包不支持应用内更新，请下载新的 .deb 或 AppImage 包后手动安装')).toBeTruthy();
  });
});
