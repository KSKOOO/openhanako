...
const CRITICAL_BUNDLED_ENTRY_CANDIDATES = [
  "server/index.js", // Linux source runtime（更稳定，避免 bundle 运行时崩溃）
  "bundle/index.js", // 兼容现有打包结构 / 其他平台
];
...
async function ensureServerFilesReady(serverRoot, opts = {}) {
  ...
  const checkOnce = () => {
    ...
    const hasServerEntry = CRITICAL_BUNDLED_ENTRY_CANDIDATES.some((file) => {
      try {
        fs.accessSync(path.join(serverRoot, ...file.split("/")), fs.constants.R_OK);
        return true;
      } catch {
        return false;
      }
    ...
    if (!hasServerEntry) {
      missing.push(CRITICAL_BUNDLED_ENTRY_CANDIDATES.join(" or "));
    }
...
module.exports = {
  ...
  CRITICAL_BUNDLED_ENTRY_CANDIDATES,
