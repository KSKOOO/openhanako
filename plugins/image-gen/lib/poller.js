...
function generationMetaType(type) {
  const normalized = String(type || "image").trim();
  if (normalized === "video" || normalized === "audio" || normalized === "music") {
...
export class Poller {
  ...
  start() {
    ...
    for (const task of pending) {
      ...
      this._bus.request("deferred:register", {
        ...
        meta: { type: generationMetaType(task.type), prompt: task.prompt },
      ...
      this._bus.request("task:register", {
        ...
        meta: { type: generationMetaType(task.type) },
      }).catch(() => {});
    }
    if (pending.length > 0) {
  ...
  async _checkTask(taskId, task) {
    ...
    const ctx = {
      ...
      dataDir: pathJoin(this._generatedDir, ".."),
      bus: this._bus,
      log: this._log,
      task,
    };
