...
- `image`：单张参考图路径（旧参数，图生图、图片编辑、风格迁移时可传入）
- `images`：多张参考图路径数组，按顺序传入；例如服装图+模特底图、主体图+风格图等多参考图任务
...
| 编辑/修改图片 | "把帽子去掉" | generate-image + image/images 参数 | prompt 写编辑指令 |
| 参考图生新图 | "参考这个风格画一套icon" | generate-image + image/images 参数 | prompt 说明参考什么 + 要生成什么；多参考图优先用 images 数组 |
