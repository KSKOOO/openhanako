...
const useSourceServer = platform === "linux";
const runtimeServerEntryRel = useSourceServer
  ? path.join("server", "index.js")
  : path.join("bundle", "index.js");
const runtimeServerEntryPosix = runtimeServerEntryRel.split(path.sep).join("/");
...
// Linux source runtime currently has a Hono private-field crash on Node 22
// after the full server import graph is loaded. Node 20 LTS is stable here,
// and the codebase only requires Node >= 20.
const NODE_VERSION = useSourceServer ? "v20.19.4" : "v22.16.0";
const NODE_VERSION_FOR_NPM = NODE_VERSION.replace(/^v/, "");
...
if (!fs.existsSync(cachedNodeBin)) {
  ...
  } else if (process.env.CURL || fs.existsSync("/usr/bin/curl") || fs.existsSync("/bin/curl")) {
    execSync(`curl -L -o "${cachedArchive}" "${url}"`, { stdio: "inherit" });
  } else {
    execSync(`wget -O "${cachedArchive}" "${url}"`, { stdio: "inherit" });
  }

  if (isWin) {
    execSync(`powershell -command "Expand-Archive -Path '${cachedArchive}' -DestinationPath '${cacheDir}' -Force"`, { stdio: "inherit" });
  } else {
    const linkExcludes = isHostWin
      ? ` --exclude "${nodeDirName}/bin/npm" --exclude "${nodeDirName}/bin/npx" --exclude "${nodeDirName}/bin/corepack"`
      : "";
    execSync(`tar xzf "${cachedArchive}"${linkExcludes} -C "${cacheDir}"`, { stdio: "inherit" });
  }
...
if (!isWin && fs.existsSync(cachedNodeBin)) {
  fs.chmodSync(cachedNodeBin, 0o755);
}
...
const canRunTargetNode = process.platform === platform;
const targetEnv = {
  ...process.env,
  NODE_ENV: "production",
  npm_config_platform: platform,
  npm_config_arch: arch,
  npm_config_target: NODE_VERSION_FOR_NPM,
  npm_config_runtime: "node",
  npm_config_nodedir: path.join(cacheDir, nodeDirName),
  ...(platform === "linux" ? { npm_config_libc: "glibc" } : {}),
  PATH: canRunTargetNode ? `${targetNodeDir}${path.delimiter}${process.env.PATH}` : process.env.PATH,
};
function runWithTargetNode(cmd, opts = {}) {
  const nodeBin = canRunTargetNode ? cachedNodeBin : process.execPath;
  execSync(`"${nodeBin}" ${cmd}`, {
    cwd: outDir,
    stdio: "inherit",
    env: targetEnv,
    ...opts,
...
function copyDirIfExists(src, dest, label) {
  if (!fs.existsSync(src)) {
    console.warn(`[build-server] warning: ${label} not found, skipping`);
    return;
  }
...
if (!fs.existsSync(viteCli)) {
  throw new Error(`[build-server] vite CLI not found: ${viteCli}`);
}
execSync(`"${process.execPath}" "${viteCli}" build --config vite.config.server.js`, {
  cwd: ROOT,
  stdio: "inherit",
});
...
// Runtime plugin files keep relative imports to ../../../shared/*.js.
// Copy shared helpers so bundled server resources can load plugins directly.
const sharedSrc = path.join(ROOT, "shared");
if (fs.existsSync(sharedSrc)) {
  fs.cpSync(sharedSrc, path.join(outDir, "shared"), { recursive: true });
  console.log("[build-server]   shared/");
}

// Plugin route modules are copied as runtime files and keep relative imports like
// "../../../server/hono-helpers.js", which resolve to outDir/server/hono-helpers.js.
const serverRuntimeOutDir = path.join(outDir, "server");
fs.mkdirSync(serverRuntimeOutDir, { recursive: true });
const honoHelpersSrc = path.join(ROOT, "server", "hono-helpers.js");
if (fs.existsSync(honoHelpersSrc)) {
  fs.copyFileSync(honoHelpersSrc, path.join(serverRuntimeOutDir, "hono-helpers.js"));
  console.log("[build-server]   server/hono-helpers.js");
} else {
  console.warn("[build-server] warning: server/hono-helpers.js not found, skipping");
}

if (useSourceServer) {
  console.log("[build-server] linux source runtime enabled; copying full runtime source tree...");
  copyDirIfExists(path.join(ROOT, "server"), path.join(outDir, "server"), "server");
  copyDirIfExists(path.join(ROOT, "core"), path.join(outDir, "core"), "core");
  copyDirIfExists(path.join(ROOT, "hub"), path.join(outDir, "hub"), "hub");
  copyDirIfExists(path.join(ROOT, "lib"), path.join(outDir, "lib"), "lib");
  copyDirIfExists(path.join(ROOT, "shared"), path.join(outDir, "shared"), "shared");
  copyDirIfExists(path.join(ROOT, "plugins"), path.join(outDir, "plugins"), "plugins");
  copyDirIfExists(path.join(ROOT, "desktop", "src", "shared"), path.join(outDir, "desktop", "src", "shared"), "desktop/src/shared");
}
...
if (useSourceServer) {
  ...
  if (fs.existsSync(rootLockfile)) {
    fs.copyFileSync(rootLockfile, path.join(outDir, "package-lock.json"));
  }
} else {
  const externalPkg = {
    name: "hanako-server",
...
// ── 5. 用目标 Node 的 npm 安装运行时依赖 ──
// 不加 --ignore-scripts：better-sqlite3 的 install 脚本需要跑
// （prebuild-install 下载正确 ABI 的预编译二进制）
// Linux source runtime 会复制完整源码树，因此需要完整 production deps。
// 仍使用 npm install，兼容精简 package.json 和锁文件共存两种模式。
console.log("[build-server] installing runtime dependencies...");
runWithTargetNode(`"${cachedNpmCli}" install --omit=dev`);

// better-sqlite3 must be rebuilt against the bundled Node.js headers.
// On Linux desktops with a system nodejs-dev package installed, node-gyp may
// otherwise pick /usr/include/nodejs and produce an ABI-mismatched addon.
console.log("[build-server] rebuilding better-sqlite3 against bundled Node.js headers...");
runWithTargetNode(`"${cachedNpmCli}" rebuild better-sqlite3 --build-from-source`);

console.log("[build-server] verifying better-sqlite3 ABI...");
runWithTargetNode(`-e "await import('better-sqlite3'); console.log('better-sqlite3 abi ok', process.version, process.versions.modules)"`);
...
let fileList;
if (useSourceServer) {
  console.log("[build-server] source runtime enabled; skipping nft prune to preserve dynamically loaded plugin dependencies");
  fileList = null;
} else if (!canRunTargetNode) {
  console.warn("[build-server] cross-platform build detected, skipping nft prune");
  fileList = null;
} else {
...
try {
  ({ fileList } = await nodeFileTrace(
    [path.join(outDir, runtimeServerEntryRel)],
...
// ── 10. Wrapper 脚本 ──
if (isWin) {
  const runtimeServerEntryWindows = runtimeServerEntryRel.split(path.sep).join("\\");
  fs.writeFileSync(
    path.join(outDir, "hana-server.cmd"),
    `@echo off\r\nset "HANA_ROOT=%~dp0"\r\nset "HANA_SERVER_ENTRY=%~dp0${runtimeServerEntryWindows}"\r\n"%~dp0hana-server.exe" "%~dp0bootstrap.js" %*\r\n`,
  );
} else {
  ...
  fs.writeFileSync(wrapper, [
    ...
    `export HANA_SERVER_ENTRY="$DIR/${runtimeServerEntryPosix}"`,
