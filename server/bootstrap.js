...
function resolveDefaultServerEntry(rootDir) {
  const sourceEntry = path.join(rootDir, "server", "index.js");
  if (fs.existsSync(sourceEntry)) return sourceEntry;
  return path.join(rootDir, "bundle", "index.js");
}

const serverEntry = process.env.HANA_SERVER_ENTRY || resolveDefaultServerEntry(hanaRoot);
