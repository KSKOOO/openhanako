...
const REMOTE_ENABLED = /^(1|true|yes|on)$/i.test(String(process.env.HANA_ALLOW_REMOTE || "").trim());
...
const configuredCorsOrigins = splitCorsOrigins(process.env.HANA_CORS_ORIGIN);
const allowAnyCorsOrigin = configuredCorsOrigins.includes("*") || (REMOTE_ENABLED && configuredCorsOrigins.length === 0);
const host = process.env.HANA_HOST || (REMOTE_ENABLED ? "0.0.0.0" : "127.0.0.1");

function isAllowedCorsOrigin(origin) {
  if (!origin) return false;
  if (allowAnyCorsOrigin) return /^https?:\/\/[^/]+$/i.test(origin);
...
// Hono `use()` 在当前 Linux 打包运行时会异常触发 private route helper 崩溃，
// 这里改用 `all("*", middleware)` 保持同等中间件语义。
...
app.get("/api/health", async (c) => {
  ...
  return c.json({
    ...
    avatars,
    remoteAccess: REMOTE_ENABLED,
    bindHost: host,
    corsOrigins: allowAnyCorsOrigin ? ["*"] : configuredCorsOrigins,
  });
});
...
try {
  ...
  if (REMOTE_ENABLED) {
    console.log("[server] remote access enabled; bind host allows LAN/mobile clients");
    if (configuredCorsOrigins.length === 0) {
      console.log("[server] remote CORS default is permissive because HANA_ALLOW_REMOTE=1");
    }
  }
